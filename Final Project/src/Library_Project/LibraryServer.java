	  ...
	  new Thread(() -> {
		  try {
			  ...
			  while (true) {
				  ...
				  String clientBookName = inputFromClient.readUTF();
				  String clientISBN = inputFromClient.readUTF();
				  String clientEmail = inputFromClient.readUTF();
				  String clientPhoneNumber = inputFromClient.readUTF();
      
				  Platform.runLater(() -> {
					  data.add(new LibraryLog(clientFirstName, clientLastName, clientBookName, clientISBN, clientEmail, clientPhoneNumber));
